...
raw_data = urllib.request.urlopen(nse_url)
...
for company_symbol in symbols_array:
    ...
    for account_to_get in accounts_to_get:
        ft_url = "http://markets.ft.com/data/equities/tearsheet/financials?s=%s:LAG&subview=%s" % (
        company_symbol, account_to_get)

        # fetch raw html data
        connection = urllib.request.urlopen(ft_url)
